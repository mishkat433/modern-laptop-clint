...
const AllUserTable = ({ user, index, verifyHandle, makeAdminHandle, deleteUserHandle }) => {
    return (
        <tr className='text-center'>
            ...
            <td>{
                user?.verify === 'verified' ? <button className='btn btn-sm btn-success'>Verified</button> : <button onClick={() => verifyHandle(user?._id)} className='btn btn-sm bg-orange-500 border-none'>unverified</button>
            }</td>
